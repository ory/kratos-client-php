...
class PublicApi
{
    ...
    {
        $request = $this->whoamiRequest($authorization);

        try {
            $options = $this->createHttpClientOption($cookie);
            try {
                $response = $this->client->send($request, $options);
            } catch (RequestException $e) {
                throw new ApiException(
    ...
    public function whoamiAsyncWithHttpInfo($cookie = null, $authorization = null)
    {
        ...
        return $this->client
            ->sendAsync($request, $this->createHttpClientOption($cookie))
            ->then(
                function ($response) use ($returnType) {
                    $responseBody = $response->getBody();
                    if ($returnType === '\SplFileObject') {
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function whoamiRequest($authorization = null)
    {
     ...
     */
    protected function createHttpClientOption($cookie = null)
    {
        $options = [];
        ...
        if ($cookie) {
            $options[RequestOptions::COOKIES] = CookieJar::fromArray( [
                $this->getConfig()->getCookieName() => $cookie
            ], $this->getConfig()->getCookieHost() );
        }
